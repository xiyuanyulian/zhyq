...
export class SimpleTableComponent implements OnInit, OnChanges, OnDestroy {
  ...
  /** `header` 标题 */
  @ContentChild('header', {static: false}) header: TemplateRef<void>;
  /** 额外 `body` 内容 */
  @ContentChild('body', {static: false}) body: TemplateRef<void>;
  /** `footer` 底部 */
  @ContentChild('footer', {static: false}) footer: TemplateRef<void>;
  /** `expand` 可展开，当数据源中包括 `expand` 表示展开状态 */
  @ContentChild('expand', {static: false})
  ...
  private updateColumns() {
    ...
    for (const item of copyColumens) {
      ...
      if (!item.filterIcon) item.filterIcon = `filter`;
